  ...
  function replaceProfessor(professor){
    ...
    function walkText(node) {
      if (node.nodeType == 1) {
        ...
        // For some reason, only works by changing professor name to last + first initial, full name freezes
        ...
        node.innerHTML = node.innerHTML.replace(
          re,
          `<a href=http://www.ratemyprofessors.com/ShowRatings.jsp?tid=${id}>${newName}</a>` //help: variablilze with variable id
        );
      }
  ...
  replaceProfessor('Eric Hernandez');
